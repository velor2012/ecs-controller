...
require_once 'DdnsScheduler.php';
...
class AliyunTrafficCheck
{
    ...
    public function getConfigForFrontend()
    {
        ...
        $config = [
            ...
            'Ddns' => [
                ...
                'schedule_linking_enabled' => ($settings['ddns_schedule_linking_enabled'] ?? '0') === '1',
                'cloudflare' => [
    ...
    public function monitor()
    {
        ...
        // DDNS 调度检查（Phase 1 骨架）
        if (isset($this->ddnsService)) {
            try {
    ...
    public function getScheduleRulesForFrontend(): array
    {
        if ($this->initError) {
            return ['error' => $this->initError];
        }
    ...
    public function saveScheduleRulesFromFrontend(array $data): array
    {
        if ($this->initError) {
    ...
    public function triggerScheduleSwitch(int $ruleIndex): array
    {
        ...
        try {
            ...
            if (!isset($rules[$ruleIndex])) {
                return ['success' => false, 'message' => '规则索引不存在'];
            }
            $scheduler = new DdnsScheduler($this->configManager, $this->db, $this->ddnsService, $this);
            $switched = $scheduler->processRuleForced($rules[$ruleIndex]);
            if ($switched) {
                return ['success' => true];
            }
            return ['success' => false, 'message' => '没有满足当前时间段的实例，请检查调度条目配置'];
        } catch (Exception $e) {
            return ['success' => false, 'message' => $e->getMessage()];
        }
    }

    public function getScheduleLogs(int $limit = 50): array
    {
        if ($this->initError) {
            return ['error' => $this->initError];
        }
        try {
            return $this->db->getLogsByTypes(['schedule'], $limit);
        } catch (Exception $e) {
            return ['error' => $e->getMessage()];
        }
    }

    public function setScheduleAutoPause(bool $paused): array
    {
        if ($this->initError) {
            return ['success' => false, 'error' => $this->initError];
        }
        $state = $this->configManager->getScheduleState();
        $state['auto_schedule_paused'] = $paused;
        $this->configManager->saveScheduleState($state);
        return ['success' => true, 'auto_schedule_paused' => $paused];
    }

    public function getScheduleDnsStatus(): array
    {
        if ($this->initError) {
            return ['error' => $this->initError];
        }
        ...
        $rule = $rules[0];
        $domain = $this->getScheduleRecordName($rule);
        if (empty($domain)) {
            return ['hasSchedule' => false, 'message' => '调度域名未配置'];
        }
        try {
            $existing = $this->ddnsService->findScheduleRecord($domain);
            if (!$existing) {
                return ['hasSchedule' => true, 'domain' => $domain, 'resolved' => false, 'message' => 'DNS记录不存在'];
            }
            $ip = $existing['content'] ?? '';
            $accounts = $this->configManager->getAccounts();
            $matched = null;
            foreach ($accounts as $a) {
                $pip = trim((string)($a['public_ip'] ?? ''));
                $eip = trim((string)($a['eip_address'] ?? ''));
                if (($pip !== '' && $pip === $ip) || ($eip !== '' && $eip === $ip)) {
                    $matched = $a;
                    break;
                }
            }
            return [
                ...
                'resolved' => true,
                'ip' => $ip,
                'instance_id' => $matched['instance_id'] ?? null,
                'instance_remark' => $matched['remark'] ?? null,
                'proxied' => !empty($existing['proxied']),
            ];
        } catch (Exception $e) {
            return ['hasSchedule' => true, 'domain' => $domain, 'resolved' => false, 'message' => $e->getMessage()];
        }
    }

    private function getScheduleRecordName(array $rule): string
    {
        $prefix = trim((string)($rule['domain_prefix'] ?? ''));
        $rootDomain = $this->configManager->get('ddns_domain', '');
        if ($prefix !== '' && $rootDomain !== '') {
            return strtolower($prefix . '.' . $rootDomain);
        }
        if (!empty($rule['domain'])) {
            return strtolower(trim($rule['domain']));
        }
        return '';
    }
