...
class ConfigManager
{
    ...
    private function saveDdnsSettings($ddns)
    {
        ...
        $this->saveSetting('ddns_schedule_linking_enabled', !empty($ddns['schedule_linking_enabled']) ? '1' : '0');
    }
    ...
    {
        $raw = $this->get('ddns_schedule_rules', '');
        if ($raw === '') {
            return [];
        }
        $decoded = json_decode($raw, true);
    ...
    public function saveScheduleRules(array $rules): void
    {
        $this->saveSetting('ddns_schedule_rules', json_encode($rules, JSON_UNESCAPED_UNICODE));
    }

    public function getScheduleState(): array
    {
        $raw = $this->get('ddns_schedule_state', '');
        if ($raw === '') {
            return [];
        }
        $decoded = json_decode($raw, true);
    ...
    public function saveScheduleState(array $state): void
    {
